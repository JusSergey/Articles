...
BannerDisplay.prototype.init = function(data, container, config) {
    ...
    if (this.properties.width < 2 || this.properties.height < 2) {
        backgroundColor.useBorder = false;
    }
    if (backgroundColor.useBorder) {
        this.displayContainer.style.left = '-1px';
        this.displayContainer.style.top = '-1px';
    }
    this.applyBackground(this.container, backgroundColor);
...
BannerDisplay.prototype.initSlides = function(elements) {
    ...
    this.overflowSlide.init(overflowSlide, this.displayContainer);
    ...
    for (i = 0; slideData = slidesData[i]; i++) {
        ...
        slide.banner = this;
        slide.init(slideData, this.displayContainer);
...
ButtonDisplay.prototype.init = function(elementData) {
    ...
    if (
        typeof properties.labelStyle.fontSize !== 'string' ||
        (typeof properties.labelStyle.fontSize === 'string' && properties.labelStyle.fontSize.indexOf('px') === -1)
    ) {
        properties.labelStyle.fontSize = properties.labelStyle.fontSize + "px";
    }
    if (
        typeof properties.labelStyle.letterSpacing !== 'string' ||
        (typeof properties.labelStyle.letterSpacing === 'string' &&
            properties.labelStyle.letterSpacing.indexOf('px') === -1)
    ) {
        properties.labelStyle.letterSpacing = properties.labelStyle.letterSpacing + "px";
    }
    if (typeof properties.labelStyle.fontFamily === 'string' && properties.labelStyle.fontFamily.indexOf('"') === -1) {
